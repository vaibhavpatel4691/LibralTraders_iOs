...
class CompareListViewController: UIViewController {
    ...
    @IBOutlet weak var heightOfTable: UITableView!
    ...
    func hitRequest() {
        ...
        compareListViewModel.callingHttppApi { success,jsonResponse in
            if self.compareListViewModel.compareListDataModel.productList.count > 0 {
                ...
                for i in 0..<(self.compareListViewModel.compareListDataModel.attributeValueList?.count ?? 0) {
                    self.compareListViewModel.attributesHeightList.append( self.compareListViewModel.getMaxHeightCell(values:  self.compareListViewModel.compareListDataModel.attributeValueList?[i].value ?? [""]))
                }
