    ...
    @objc private func didTapCountryView() {
        ...
        if #available(iOS 12.0, *) {
            if self.traitCollection.userInterfaceStyle == .dark {
                ...
                datePicker?.titleTextAttributes = [NSAttributedString.Key.foregroundColor: AppStaticColors.darkItemTintColor]
                datePicker?.toolbarBackgroundColor = AppStaticColors.darkPrimaryColor
            } else {
                datePicker?.titleTextAttributes = [NSAttributedString.Key.foregroundColor: AppStaticColors.itemTintColor]
                datePicker?.toolbarBackgroundColor = AppStaticColors.primaryColor
            }
        } else {
            datePicker?.titleTextAttributes = [NSAttributedString.Key.foregroundColor: AppStaticColors.itemTintColor]
            datePicker?.toolbarBackgroundColor = AppStaticColors.primaryColor
