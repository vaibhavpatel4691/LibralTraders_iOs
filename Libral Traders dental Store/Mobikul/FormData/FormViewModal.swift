        ...
        for item in self.formItems {
            if item.keyType.count > 0, let value = item.value {
                ...
                if let _ = dict[item.keyType], let value = value as? String {
                    ...
                    if item.keyType == "websiteIds" {
                        print(item.keyType, item.multiData)
                        var data = [String]()
                        for val in item.multiData {
                            if val.check ?? false {
                                data.append(val.id ?? "" )
                            }
                        }
                        do {
                            let jsonWebsiteIdData =  try JSONSerialization.data(withJSONObject: data, options: .prettyPrinted)
                            let jsonWebsiteIdString:String = NSString(data: jsonWebsiteIdData, encoding: String.Encoding.utf8.rawValue)! as String
                            // params["websiteIds"] = jsonWebsiteIdString
                            dict[item.keyType] = jsonWebsiteIdString//"\(data)"
                        } catch {
                            print("")
                        }
                    } else {
...
enum FormItemCellType {
    ...
    func dequeueCell(for tableView: UITableView, at indexPath: IndexPath) -> UITableViewCell {
        ...
        case .descriptionView:
            ...
            case .multi:
            cell = tableView.dequeueReusableCell(for: indexPath,cellType: FormMultiCheckTableViewCell.self)
