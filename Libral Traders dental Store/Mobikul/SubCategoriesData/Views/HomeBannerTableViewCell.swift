...
extension HomeBannerTableViewCell: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if type == "subCategory" {
            return CGSize(width: AppDimensions.screenWidth - 116, height: 2*(AppDimensions.screenWidth - 116 ) / 3)
          // return CGSize(width: AppDimensions.screenWidth - 116, height: 3*(AppDimensions.screenWidth - 116 ) / 8)
        } else if type == "tabCategory" {
            pageControlView.center = CGPoint(x: AppDimensions.screenWidth/2 - 58, y: 20/2)
            return CGSize(width: AppDimensions.screenWidth - 116, height: 3*(AppDimensions.screenWidth - 116 ) / 8)
        } else {
