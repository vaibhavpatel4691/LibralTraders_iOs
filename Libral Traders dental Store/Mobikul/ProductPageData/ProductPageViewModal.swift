...
class ProductPageViewModal: NSObject {
    ...
    var callBackQuickOrderToCart: ((_ check: Bool)-> Void)?
    #endif
    ...
    func callingHttppApi(completion: @escaping (Bool) -> Void) {
        ...
        case .addToCart:
            ...
            if let relatedArr = self.model.relatedProductList {
                let productArr = (relatedArr.filter({$0.selected == true})).map({$0.entityId})
                requstParams["relatedProducts"] = productArr.convertArrayToString()
            }
...
//            }
        ...
        #if MARKETPLACE || BTOB
        if let sellerInfo = model.sellerInfo, (sellerInfo.sellerId != "" && sellerInfo.sellerId != "0"), (sellerInfo.displaySellerInfo ?? true) {
            self.productItems.append(ProductViewModalSellerInfoItem(sellerInfo: sellerInfo))
        }
        #endif
        #if MARKETPLACE || BTOB || HYPERLOCAL
        if let reportVal = model.productReportDataValues {
            if (Defaults.customerToken != nil) {
                if  reportVal.showReportProduct ?? false {
                    self.productItems.append(ProductViewModalReportItem())
                }
            } else {
                if reportVal.guestCanReport ?? false && reportVal.showReportProduct ?? false {
                    self.productItems.append(ProductViewModalReportItem())
                }
            }
...
extension ProductPageViewModal {
    ...
    func shareClicked() {
        if let url = model.productUrl {
            delegate?.shareClicked(productLink: url)
        }
    }
...
extension ProductPageViewModal: AddToCartProduct {
    ...
    //MARK:- quick product add to cart
    #if BTOB
    func quickAddToCartProduct(cart: Bool) {
        if self.checkingCartOptionsValidity() {
            moveToCart = cart
            self.whichApiCall = .addToCart
            self.callingHttppApi { (success) in
                self.callBackQuickOrderToCart?(success)
            }
        }
    }
    #endif
...
extension ProductPageViewModal: GettingBundleData {
...
extension ProductPageViewModal: GettingCustomData {
