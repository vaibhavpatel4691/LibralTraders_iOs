...
class ProductTopImagesTableViewCell: UITableViewCell {
    ...
    override func layoutSubviews() {
        if let arType = arType {
            if (arType == "3D" || arType == "2D") && !(arUrl.isEmpty){
    ...
    @IBAction func arBtnClicked(_ sender: Any) {
            if let arType = arType  {
                if arType == "3D" {
                    ...
                    let nextController = PngModelViewController.instantiate(fromAppStoryboard: .product)
                    nextController.modalPresentationStyle = .fullScreen
                    nextController.arUrl = arUrl
                    if arUrl == "" {
                        ShowNotificationMessages.sharedInstance.warningView(message: "Ar image not found")
                    } else {
                        self.viewContainingController?.present(nextController, animated: true)
                    }
