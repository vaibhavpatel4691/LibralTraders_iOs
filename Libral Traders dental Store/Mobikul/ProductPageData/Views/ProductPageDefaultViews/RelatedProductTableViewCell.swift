    ...
    func theme() {
        if #available(iOS 12.0, *) {
            if self.traitCollection.userInterfaceStyle == .dark {
                viewAllBtn.setTitleColor(AppStaticColors.darkButtonTextColor, for: .normal)
                viewAllBtn.backgroundColor = AppStaticColors.darkButtonBackGroundColor
            } else {
                viewAllBtn.setTitleColor(AppStaticColors.buttonTextColor, for: .normal)
                viewAllBtn.backgroundColor = AppStaticColors.buttonBackGroundColor
            }
        } else {
            viewAllBtn.setTitleColor(AppStaticColors.buttonTextColor, for: .normal)
            viewAllBtn.backgroundColor = AppStaticColors.buttonBackGroundColor
...
extension RelatedProductTableViewCell: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: ProductsCollectionViewCell.identifier, for: indexPath) as? ProductsCollectionViewCell {
            if (relatedCheck) && ((self.obj?.model.relatedProductList?[indexPath.row].configurableData?.attributes.count ?? 0) == 0){
                cell.relatedProductCheck.isHidden = false
                cell.relatedProductCheckImg.isHidden = false
                if (self.obj?.model.relatedProductList?[indexPath.row].selected ?? false) {
                    cell.relatedProductCheckImg.image = UIImage(named: "sharp-checkbox-fill")
                } else {
                    cell.relatedProductCheckImg.image = UIImage(named: "sharp-checkbox-empty")
                }
            } else {
                cell.relatedProductCheck.isHidden = true
                cell.relatedProductCheckImg.isHidden = true
            }
            ...
            cell.relatedProductCheckImg.addTapGestureRecognizer {
                if (self.obj?.model.relatedProductList?[indexPath.row].selected ?? false) {
                    self.obj?.model.relatedProductList?[indexPath.row].addRelatedItem(check: false)
                    self.relatedList[indexPath.row].addRelatedItem(check: false)
                    if let list =  self.obj?.model.relatedProductList {
                        self.relatedList = list
                    }
                    cell.relatedProductCheckImg.image = UIImage(named: "sharp-checkbox-empty")
                } else {
                    self.obj?.model.relatedProductList?[indexPath.row].addRelatedItem(check: true)
                    self.relatedList[indexPath.row].addRelatedItem(check: true)
                    if let list =  self.obj?.model.relatedProductList {
                        self.relatedList = list
                    }
                    cell.relatedProductCheckImg.image = UIImage(named: "sharp-checkbox-fill")
