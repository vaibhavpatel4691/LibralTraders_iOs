    ...
    var item: ProductPageModel! {
        didSet {
            ...
            if productType == "bundle" {
                ...
                if let arr = item.configurableData?.optionPrices?.filter({ ($0.product ?? "") == optionProductId }), let currency = Defaults.currency, let amount = arr.first?.finalPrice?.amountInDouble, let stringAmount = arr.first?.finalPrice?.amount {
                    
                    let price = NetworkManager.sharedInstance.priceFormatter(amount, decimalSymbol: item.priceFormat?.decimalSymbol ?? "", grouplength: "\(item.priceFormat?.groupLength ?? 0)", groupsymbol: item.priceFormat?.groupSymbol ?? "", pattern: item.priceFormat?.pattern ?? "", precision: "\(item.priceFormat?.precision ?? 0)")
                    if price != "" {
                        self.priceLabel.text = price
                    } else {
                       self.priceLabel.text = currency + stringAmount
                    }
                }
            }
        }
