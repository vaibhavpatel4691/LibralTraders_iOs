...
class GroupProductTableViewCell: UITableViewCell {
    ...
    var index = ""
    var section = 0
    var specialPrice: NSMutableAttributedString!
    ...
    var item: CartProducts! {
        didSet {
            ...
            if item.isInRange{
                priceLabel.text = item.specialPrice
                specialPrice = NSMutableAttributedString(string: item.formattedFinalPrice)
                specialPrice.addAttribute(NSAttributedString.Key.strikethroughStyle, value: 2, range: NSRange(location: 0, length: specialPrice.length))
                strikePrice.attributedText = specialPrice
