...
extension ProductPageViewModal: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .report:
            return self.loadProductReport(tableView: tableView, item: productItems[indexPath.section], indexPath: indexPath)
            #endif
        #if MARKETPLACE || BTOB
        case .sellerInfo:
            return self.loadSellerInfo(tableView: tableView, item: productItems[indexPath.section], indexPath: indexPath)
        #endif
        case .none:
        ...
        case .reviews:
            if !collapseReviews, let footerView = tableView.dequeueReusableHeaderFooterView(withIdentifier: ActionButtonFooterView.identifier) as? ActionButtonFooterView {
                if #available(iOS 12.0, *) {
                    if tableView.viewContainingController?.traitCollection.userInterfaceStyle == .dark {
                        footerView.btn.backgroundColor =  AppStaticColors.darkButtonBackGroundColor
                        footerView.btn.setTitleColor(AppStaticColors.darkButtonTextColor, for: .normal)
                    } else {
                        footerView.btn.backgroundColor =  AppStaticColors.buttonBackGroundColor
                        footerView.btn.setTitleColor(AppStaticColors.buttonTextColor, for: .normal)
            ...
            if #available(iOS 12.0, *) {
                if tableView.viewContainingController?.traitCollection.userInterfaceStyle == .dark {
                    if self.addedToWishlist {
                        let image = UIImage(named: "ic_wishlist_fill")?.withRenderingMode(.alwaysTemplate)
                        cell.wishlishtBtn.setImage(image, for: .normal)
                        cell.wishlishtBtn.tintColor = AppStaticColors.darkButtonBackGroundColor

                    } else {
                        let image = UIImage(named: "ic_wishlist")?.withRenderingMode(.alwaysTemplate)
                        cell.wishlishtBtn.setImage(image, for: .normal)
                        cell.wishlishtBtn.tintColor = AppStaticColors.darkButtonBackGroundColor
                    ...
                    if self.addedToWishlist {
                        let image = UIImage(named: "ic_wishlist_fill")?.withRenderingMode(.alwaysTemplate)
                        cell.wishlishtBtn.setImage(image, for: .normal)
                        cell.wishlishtBtn.tintColor = AppStaticColors.buttonBackGroundColor
...
//                    if self.addedToWishlist {
//                        if self.configDict.count != 0{
//                            Analytics.logEvent("ADD_TO_WISHLIST", parameters: ["productid":self.productId ,"name":self.model.name ?? "","option":self.configDict.convertDictionaryToString() ?? [:]])
//                        }else{
//                            Analytics.logEvent("ADD_TO_WISHLIST", parameters: ["productid":self.productId ,"name":self.model.name ?? ""])
//                        }
//
//
...
//                    }
            ...
            cell.callShare = { 
                ...
                if let url = self.model.productUrl {
                let vc = UIActivityViewController(activityItems: [url], applicationActivities: nil)
               
                tableView.viewContainingController?.present(vc, animated: true, completion: nil)
                }
...
//        return cell
    ...
    func loadShortDescription(tableView: UITableView, item: ProductViewModalItem, indexPath: IndexPath) -> UITableViewCell {
        if let cell: ProductShortDescriptionTableViewCell = tableView.dequeueReusableCell(with: ProductShortDescriptionTableViewCell.self, for: indexPath) {
            ...
            if let item = item as? ProductViewModalTierPriceItem {
                cell.selectionStyle = .none
                cell.desc = item.tierPrice
                cell.descLbl.text = item.tierPrice
    ...
    //MARK:- Reprt product
     #if MARKETPLACE || BTOB || HYPERLOCAL
    func loadProductReport(tableView: UITableView, item: ProductViewModalItem, indexPath: IndexPath) -> UITableViewCell {
        if let cell: ReportSectionTableViewCell = tableView.dequeueReusableCell(with: ReportSectionTableViewCell.self, for: indexPath) {
            if let item = item as? ProductViewModalReportItem {
                cell.theme()
                cell.selectionStyle = .none
                #if MARKETPLACE || BTOB || HYPERLOCAL
                cell.reportLbl.addTapGestureRecognizer {
                    let nextController = ReportFlagViewController.instantiate(fromAppStoryboard: .seller)
                    nextController.reportData = self.model.productReportDataValues
                    nextController.checkReportType = .product
                    nextController.productId = self.productId
                    tableView.viewContainingController?.navigationController?.pushViewController(nextController, animated: true)
                }
                #endif
    ...
    #endif
    #if MARKETPLACE || BTOB
    func loadSellerInfo(tableView: UITableView, item: ProductViewModalItem, indexPath: IndexPath) -> UITableViewCell {
        if let cell: ProductSellerInfoTableViewCell = tableView.dequeueReusableCell(with: ProductSellerInfoTableViewCell.self, for: indexPath),
            let item = item as? ProductViewModalSellerInfoItem {
            cell.selectionStyle = .none
    ...
    func loadBuyNow(tableView: UITableView, item: ProductViewModalItem, indexPath: IndexPath) -> UITableViewCell {
        if let cell: ProductCartViewTableViewCell = tableView.dequeueReusableCell(with: ProductCartViewTableViewCell.self, for: indexPath),
            ...
            cell.cartClick = {
                //MARK:- ADD_TO_CART Analytics

                 Analytics.setScreenName("ProductPage", screenClass: "ProductPageDataViewController.class")
                if self.configDict.count != 0{
                    Analytics.logEvent("ADD_TO_CART", parameters: ["productid":self.productId ,"name":self.model.name ?? "","option":self.configDict.convertDictionaryToString() ?? [:]])
                }else{
                    Analytics.logEvent("ADD_TO_CART", parameters: ["productid":self.productId ,"name":self.model.name ?? ""])
                }
            }
            cell.theme()
            cell.selectionStyle = .none
        ...
        if let item = item as? ProductViewModalBundleItem {
            ...
            case "radio":
                if let cell: BundleRadioTableViewCell = tableView.dequeueReusableCell(with: BundleRadioTableViewCell.self, for: indexPath) {
                    ...
                    cell.bundleQtyDict = self.bundleQtyDict
                    cell.parentID = items.bundleData[indexPath.row].optionId
                    cell.value = (self.bundleDict[items.bundleData[indexPath.row].optionId] as? String) ?? ""  //items.bundleData[indexPath.row].optionValues[item.selectedRow].optionValueId
                    cell.selectionStyle = .none
                    cell.headingLabel.text = items.bundleData[indexPath.row].defaultTitle
        ...
        if let item = item as? ProductViewModalCustomOptionItem {
            ...
            case "field","arfield":
                if let cell: CustomTextFieldTableViewCell = tableView.dequeueReusableCell(with: CustomTextFieldTableViewCell.self, for: indexPath) {
                    ...
                    cell.parentID = items.customOptions[indexPath.row].optionId
                    #if !BTOB
                    cell.item = items.customOptions[indexPath.row]
                    cell.valueClosure = {arMeasure in
                        //cell.customDict[items.customOptions[indexPath.row].optionId] = arMeasure
                        self.customDict[items.customOptions[indexPath.row].optionId] = arMeasure
                       if let section = self.customSection {
                        self.reloadSectionsWithoutAnimation?(section)
                        }
                        print("option id\(items.customOptions[indexPath.row].optionId)")
                    }
                    cell.textFieldCloure = {textfieldText in
                        self.customDict[items.customOptions[indexPath.row].optionId] = textfieldText
                        if let section = self.customSection {
                        self.reloadSectionsWithoutAnimation?(section)
                        }
                    }
