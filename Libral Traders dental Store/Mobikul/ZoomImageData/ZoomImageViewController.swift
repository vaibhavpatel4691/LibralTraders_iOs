...
class ZoomImageViewController: UIViewController {
    ...
    var currentPage = 0
    var checkFirst = true
    var firstImageData: ImageGallery!
    override func viewDidLoad() {
        super.viewDidLoad()
        if images.count > 0 {
            firstImageData = images[0]
            images[0] = images[currentPage]
        }
...
//            zoomCollectionVFiew.scrollToItem(at: indexPath1, at: .right, animated: false)
            zoomCollectionVFiew.contentOffset.x = AppDimensions.screenWidth * CGFloat(currentPage)
        }
        if images.count > 0 {
            images[0] = firstImageData
        }
    }
...
//        if checkFirst && (currentPage == indexPath.row) {
