...
class CustomTableHeaderView: UIView {
    ...
    override func awakeFromNib() {
        ...
        editInfoBtn.backgroundColor = AppStaticColors.buttonBackGroundColor
        editInfoBtn.setTitleColor(AppStaticColors.buttonTextColor, for: .normal)
...
//            var config = ImageCropperConfiguration(with: image, and: .rect2x1)
...
//
            ...
            //let cropper = ImageCropperViewController.initialize(with: config) { croppedImage in
                ...
                if let data = image.jpegCompressToMaxLimit() {
                    ...
                    let fileInfo = FileInfo(fileKey: "files", fileName: fileName, fileData: data)
                    NetworkManager.sharedInstance.uploadMultipleFilesToServer(dict: [:], apiCall: self.type == "banner" ? .uploadBannerPic:.uploadProfilePic, fileInfo: [fileInfo]) { (success, response) in
                        if success {
            ...
            //self.viewContainingController?.present(cropper, animated: true, completion: nil)
