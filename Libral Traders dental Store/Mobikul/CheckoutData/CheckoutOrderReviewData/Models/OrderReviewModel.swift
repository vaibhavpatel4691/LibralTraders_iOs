...
public struct OrderReviewModel {
    ...
    public var razorpaydData: RazorpayData?
    ...
    public init(json: JSON) {
        ...
        let payments = paymentMethods.filter {(PaymentData.supportedPaymnets.contains($0.code))}
        ...
        if let item = json["razorpay_data"].array?.first{
            razorpaydData = RazorpayData(json: item)
        }else{
            razorpaydData = RazorpayData(json: json["razorpay_data"])
