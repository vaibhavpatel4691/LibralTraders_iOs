...
extension SearchViewModel: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .suggestions:
            if let item = item as? SearchViewModelSuggestionItem {
                if let cell = tableView.dequeueReusableCell(withIdentifier: "SearchPageSuggestionTableViewCell", for: indexPath)as? SearchPageSuggestionTableViewCell {
                    
                    cell.productName.text = item.products[indexPath.row].productName.html2String
                    #if GROCERY
                    cell.productImage.setImage(fromURL: item.products[indexPath.row].thumbNail, placeholder: UIImage(named: "placeholder-Grocery"))
                    #else
                    cell.productImage.setImage(fromURL: item.products[indexPath.row].thumbNail, placeholder: UIImage(named: "placeholder"))
                    #endif
                    
                    cell.productImage.contentMode = .scaleAspectFit
                if item.products[indexPath.row].hasSpecialPrice {
                    
                    cell.productPrice.text = item.products[indexPath.row].price
                    cell.specialPrice.attributedText = item.products[indexPath.row].strikePrice
                } else {
                    cell.productPrice.text = item.products[indexPath.row].price
                }
        ...
        case .categories:
            if let item = item as? SearchViewModelCategoryItem {
                if let cell = tableView.dequeueReusableCell(with: SearchCategoryTableViewCell.self, for: indexPath) {
                    ...
                    cell.layoutIfNeeded()
                    cell.categoryCollectionView.reloadData()
                    cell.collectionViewHeightConstraint.constant = cell.categoryCollectionView.contentSize.height
                    return cell
                }
            }
        }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        case .categories:
            let view = SearchHeader(frame: CGRect(x: 0, y: 0, width: AppDimensions.screenWidth, height: 50))
            view.nameLbl.text = "Popular Searches".localized
