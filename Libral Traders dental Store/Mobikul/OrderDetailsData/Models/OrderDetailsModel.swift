...
public struct OrderDetailsModel {
    ...
    var creditmemoList: [CustomerCreditMemoListData]?
    ...
    var rating: String!
    var deliveryBoys = [DeliveryBoysData]()
    ...
    public init(json: JSON) {
        ...
        if let items = json[SerializationKeys.creditmemoList].array { creditmemoList = items.map({ (value) -> CustomerCreditMemoListData in
            return CustomerCreditMemoListData(data: value)
        })
        ...
        if let items = json["deliveryBoys"].array { deliveryBoys = items.map { DeliveryBoysData(json: $0) } }
        print(deliveryBoys)
        deliveryBoys = deliveryBoys.filter({$0.isEligibleForDeliveryBoy})

    }
    
}
struct DeliveryBoysData {
    var isEligibleForDeliveryBoy: Bool!
    var picked: Bool!
    var vehicleNumber: String!
    var avatar: String!
    ...
    init (json: JSON) {
        ...
        name = json["name"].stringValue
        otp = json["otp"].stringValue
        mobile = json["mobileNumber"].string ?? json["mobile"].stringValue
        deliveryboyId = json["id"].stringValue
        customerId = json["customerId"].stringValue
        rating = json["rating"].stringValue
...
struct CustomerCreditMemoListData {
