...
class AccountInformationViewModal: NSObject {
    ...
    func callingHttppApi(completion: @escaping (Bool) -> Void,_ dict: [String: Any] = [:]) {
        ...
        NetworkManager.sharedInstance.callingHttpRequest(params: requstParams, method: verbs, apiname: apiName, currentView: UIViewController()) { [weak self] success, responseObject  in
            ...
            if success == 1 {
                ...
                if jsonResponse["success"].boolValue == true {
                    ...
                    self?.doFurtherProcessingWithResult(data: jsonResponse, completion: { success in
                        completion(success)
                    }, dict: dict)
                } else {
                ...
                NetworkManager.sharedInstance.dismissLoader()
                self?.callingHttppApi(completion: {success in
                    completion(success)
    ...
    func doFurtherProcessingWithResult(data: JSON, completion: (Bool) -> Void, dict: [String: Any]) {
        
        if apiType == "save"  {
            if let email = dict["email"] as? String, changeEmailFlag {
            ...
            self.createSwitchControl(placeholder: "Change Email".localized,
                                     isRequired: false, key: "", id: 1)
    ...
    func saveAddressClicked(completion: @escaping (Bool) -> Void) {
        ...
        if !form.isValid().0 {
            let accountInfoValue =  (form.getFormData())
            if (form.isValid().1 == "Email".localized) && (accountInfoValue["email"]as? String ?? "") != "" {
                ShowNotificationMessages.sharedInstance.warningView(message: (form.isValid().1).replacingOccurrences(of: "*", with: "") + " " + "input value is not valid".localized)
            } else {
            ...
            if let password = dict["newPassword"] as? String{
                if password !=  dict["confirmPassword"] as? String{
                    ...
                    self.callingHttppApi(completion: { _ in
                        completion(true)
                    }, dict)
                }
...
extension AccountInformationViewModal:changeDelegate {
    func passdata(index: Int, actionData: Double) {
        if index == 1{
            if actionData == 1{
