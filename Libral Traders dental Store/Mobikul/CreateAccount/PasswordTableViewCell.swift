...
class PasswordTableViewCell: UITableViewCell {
    ...
    override func awakeFromNib() {
        ...
        confirmButton.imageEdgeInsets = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0)
        ...
        if Defaults.language == "ar" {
            passwordText.semanticContentAttribute = .forceRightToLeft
            confirmPasswordTxtfld.semanticContentAttribute = .forceRightToLeft
            passwordText.textAlignment = .right
            confirmPasswordTxtfld.textAlignment = .right
        } else {
            passwordText.semanticContentAttribute = .forceLeftToRight
            confirmPasswordTxtfld.semanticContentAttribute = .forceLeftToRight
            passwordText.textAlignment = .left
            confirmPasswordTxtfld.textAlignment = .left
        }
            ...
            if newText.count>=8 {
                if newText.isValidPassword() {
                    ...
                    if (minimumCharaterLbl.text ?? "").isEmpty {
                        minimumCharaterLbl.text = "Minimum of different classes of characters in password is 3. Classes of characters: Lower Case, Upper Case, Digits".localized
