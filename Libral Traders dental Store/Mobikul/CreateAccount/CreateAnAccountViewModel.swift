...
class CreateAnAccountViewModel: NSObject {
    ...
    var orderId = ""
    var mobile = "" 
    var newAccountClousre:((_ customerToken:String)->Void)?
    ...
    func callingHttppApi(completion: @escaping (Bool) -> Void) {
        ...
        if saveData {
            ...
            if !(orderId.isEmpty) {
                requstParams["orderId"] = self.orderId
            }
            for (key, value) in signupDictionary {
                requstParams[key] = value
            }
            if socialRequiredCheck {
                if let pUrl = customerDetails.pUrl {
                   requstParams["pictureURL"] =  pUrl
                } else {
    ...
    func doFurtherProcessingWithResult(data: JSON, completion: (Bool) -> Void) {
        if !(saveData) {
            ...
            if dataModel?.signUpData?.isTaxVisible ?? false == true {
                 var taxvatString = "taxvat".localized
                if dataModel?.signUpData?.isTaxRequired ?? false {
                    taxvatString += " *"
                }
                self.createTextField(placeholder: taxvatString, isRequired: dataModel?.signUpData?.isTaxRequired ?? false, key: "taxvat")
            }
            if dataModel?.signUpData?.isGenderVisible ?? false {
                ...
                self.createSingleDropDown(placeholder: genderString, key: "gender", prifixData: genderArray, isRequired: dataModel?.signUpData?.isGenderRequired ?? false, heading: "gender".localized, image: UIImage(named: "sharp-arrow-top")!, value: genderArray.first ?? "")
            }
            ...
            if dataModel?.signUpData?.sellerRegistrationStatus ?? false {
                items.append(RegisterSellerItem())
            }
            #endif
            if socialRequiredCheck {
             } else {
            ...
            self.defaults.synchronize()
            newAccountClousre?(data["customerToken"].stringValue)
    ...
    @objc func saveDataClicked() {
        if socialRequiredCheck {
            passwordTextFieldData = "".random(length: 10)
            confirmPasswordTextFieldData = passwordTextFieldData
        }
        print(form.getFormData())
        
//        if dataModel?.signUpData?.isGenderRequired ?? false {
//            signupDictionary = form.getFormData()
//            if signupDictionary["gender"]as? String ?? "" == "" {
//                ShowNotificationMessages.sharedInstance.warningView(message: "Gender".localized + " " + "input value is not valid".localized)
//                return
//            }
//        }
        if !form.isValid().0 {
            let createAccountInfoValue =  (form.getFormData())
            if (form.isValid().1 == "Email".localized + " *") && (createAccountInfoValue["email"]as? String ?? "") != "" {
            ...
            ShowNotificationMessages.sharedInstance.warningView(message: "Enter the".localized + " " + "Password".localized)
        } else if confirmPasswordTextFieldData.isEmpty {
            ShowNotificationMessages.sharedInstance.warningView(message: "Enter the".localized + " " + "Confirm Password *".localized)
            ...
            signupDictionary["mobile"] = self.mobile
            ...
            } else if signupDictionary["gender"]as? String ?? "" == "Female".localized {
...
extension CreateAnAccountViewModel {
    ...
    func createSingleDropDown(placeholder: String, key: String = "", prifixData: [String], isRequired: Bool = false, heading: String = "", image: UIImage, value: String = "") {
        let fieldItem = FormItem(placeholder: placeholder)
...
extension CreateAnAccountViewModel: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            if let cell = tableView.dequeueReusableCell(withIdentifier: "PasswordTableViewCell", for: indexPath)as? PasswordTableViewCell {
                ...
                if #available(iOS 12.0, *) {
                    if cell.traitCollection.userInterfaceStyle == .dark {
                        cell.passwordText.textColor = UIColor.white
                        cell.confirmPasswordTxtfld.textColor = UIColor.white
                                          
                        cell.passwordController.floatingPlaceholderNormalColor = AppStaticColors.accentColor
                         cell.passwordController.inlinePlaceholderColor = AppStaticColors.accentColor
                      cell.confirmPasswordController.floatingPlaceholderNormalColor = AppStaticColors.accentColor
                      cell.confirmPasswordController.inlinePlaceholderColor = AppStaticColors.accentColor
                    } else {
                           cell.passwordText.textColor = AppStaticColors.accentColor
                                          cell.confirmPasswordTxtfld.textColor = AppStaticColors.accentColor
                    }
                } else {
                      cell.passwordText.textColor = AppStaticColors.accentColor
                                                              cell.confirmPasswordTxtfld.textColor = AppStaticColors.accentColor
        ...
        #if MARKETPLACE || HYPERLOCAL
        case .registerSeller:
            if let cell = tableView.dequeueReusableCell(withIdentifier: RegisterSellerTableViewCell.identifier) as? RegisterSellerTableViewCell {
                cell.item = registerSeller
                cell.callbackSwitchUpdate = {(isRegister) in
                    self.registerSeller = isRegister
                    tableView.reloadSections(IndexSet(arrayLiteral: indexPath.section), with: .none)
                }
                cell.callbackShopURLUpdate = {(shopURL) in
...
#if MARKETPLACE || HYPERLOCAL
class RegisterSellerItem: SignupCasesViewItem {
