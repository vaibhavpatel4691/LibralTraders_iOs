...
class NewAddressViewModel: NSObject {
    ...
    var empryStateCheck = false
    var controller: NewAddressDataViewController?
    ...
    func doFurtherProcessingWithResult(data: JSON, completion: (Bool) -> Void) {
        ...
        if dataModel.isPrefixVisible == true {
            if let names = Defaults.customerName?.components(separatedBy: " "), names.count > 1 {
                addressData?.prefix =  names[0]
                firstname = addressData?.firstname ?? names[names.count - 2]
        ...
        if dataModel.isCompanyVisible {
            var placeHolder = "Company".localized
            if dataModel.isCompanyRequired {
                placeHolder += " *"
            }
            self.createTextField(placeholder: placeHolder, isRequired: true, key: "company", value: addressData?.company)
            self.addressDictionary["company"] = addressData?.company
        }
        if dataModel.isTelephoneVisible {
            var placeHolder = "Phone Number".localized
            if dataModel.isTelephoneRequired {
                placeHolder += " *"
            }
            self.createTextField(placeholder: placeHolder, isRequired: dataModel.isTelephoneRequired, key: "telephone", value: addressData?.telephone)
            self.addressDictionary["telephone"] = addressData?.telephone
            
        }
        
        var placeHolder = "GST Number".localized
        self.createTextField(placeholder: placeHolder, isRequired: dataModel.isTelephoneRequired, key: "gstnumber", value: addressData?.gstNumber)
        self.addressDictionary["gstnumber"] = addressData?.gstNumber
        /*if dataModel.isGSTNumberVisible {
            var placeHolder = "GST Number".localized
            if dataModel.isTelephoneRequired {
                placeHolder += ""
            }
        ...
        if dataModel.isFaxVisible {
            var placeHolder = "fax".localized
            if dataModel.isFaxRequired {
                placeHolder += " *"
            }
            self.createTextField(placeholder: placeHolder, isRequired: true, key: "fax", value: addressData?.fax)
        ...
        if let regionValue = (addressData?.regionId) {
            if (regionValue.isNumeric) {
                self.addressDictionary["region_id"] = addressData?.regionId ?? dataModel.getFirstRegionId(countryId: countryData as? String ?? "")
                
            } else {
                self.addressDictionary["region_id"] = dataModel.getHyperLocalRegionId(countryId: countryData as? String ?? "", region: addressData?.regionId ?? "")
            }
        } else {
            self.addressDictionary["region_id"] = addressData?.regionId ?? dataModel.getFirstRegionId(countryId: countryData as? String ?? "")
        ...
        self.createTextField(placeholder: "State".localized + "", isRequired: false, key: "region", value: (addressData?.regionId ?? "") == "0" ? (addressData?.region ?? "") :  "")
    ...
    func saveAddressClicked(completion: @escaping (Bool) -> Void) {
        ...
        if !form.isValid().0 {
            if addressType == "Checkout" && Defaults.customerToken == nil {
                let addressValue =  (form.getFormData())
                if (form.isValid().1 == "Email Address".localized + " *") && (addressValue["email"]as? String ?? "") != "" {
                    ShowNotificationMessages.sharedInstance.warningView(message: (form.isValid().1 + " " + "input value is not valid".localized).replacingOccurrences(of: "*", with: ""))
                } else {
                    ShowNotificationMessages.sharedInstance.warningView(message: ("Enter the".localized + " " + form.isValid().1).replacingOccurrences(of: "*", with: ""))
                }
            ...
            if let regionValue = (addressDictionary["region_id"]as? String), let countryIdValue = (addressDictionary["country_id"]as? String) {
                if !(regionValue.isNumeric) {
                    self.addressDictionary["region_id"] = dataModel.getHyperLocalRegionId(countryId: countryIdValue, region: regionValue)
                    
                }
            }
            //addressDictionary["region"] = ""
            if !(self.empryStateCheck) {
                addressDictionary["region"] = ""
            } else {
            ...
            if (addressDictionary["region"]as? String) != "" {
                addressDictionary["region_id"] = "0"
            }
            if (addressDictionary["region_id"]as? String) != "0" {
                addressDictionary["region"] = ""
            }
...
extension NewAddressViewModel {
    ...
    func createCountryDropDown(placeholder: String, key: String = "", key2: String = "", countryData: [CountryData], isRequired: Bool = false, heading: String = "", image: UIImage, placeholder2: String, value: Any?,  value2: Any?) {
        ...
        fieldItem.placeholder2 = placeholder2
        fieldItem.valueCompletion = { [self, weak fieldItem] value in
            if (value?.isEmpty ?? false) || (value == "firstHit") {
                self.empryStateCheck = true
            } else {
                self.empryStateCheck = false
            }
            for i in 0..<self.form.formItems.count {
                if self.form.formItems[i].keyType == "region" && (value != "firstHit") && ((self.form.formItems[i].value as? String ?? "") != "") {
                    self.form.formItems[i].value = ""
                }
            }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cellType = self.form.formItems[indexPath.row].uiProperties.cellType {
            if self.form.formItems[indexPath.row].heading == "It's a default billing address.".localized || self.form.formItems[indexPath.row].heading == "It's a default shipping address.".localized {
                ...
                if !(self.empryStateCheck) && self.form.formItems[indexPath.row].keyType == "region" {
