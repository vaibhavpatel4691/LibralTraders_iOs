...
public struct NewAddressModel {
    ...
    public init(json: JSON) {
        ...
        self.isTelephoneRequired = json["isTelephoneRequired"].boolValue
        self.isCompanyVisible = json["isCompanyVisible"].boolValue
        self.isFaxVisible = json["isFaxVisible"].boolValue
        self.isFaxRequired = json["isFaxRequired"].boolValue
        self.isCompanyRequired = json["isCompanyRequired"].boolValue
    ...
    func getHyperLocalRegionId(countryId: String, region: String) -> String {
        if countryData.count > 0 {
            if let index = countryData.firstIndex(where: { $0.countryId == countryId }) {
                let stateData = countryData[index].states.filter({$0.code == region })
                return stateData.first?.regionId ?? ""
            }
        }
        return ""
    }
